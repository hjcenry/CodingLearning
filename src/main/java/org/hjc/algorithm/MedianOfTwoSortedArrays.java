...
public class MedianOfTwoSortedArrays {
    ...
    思路：搞清楚中位数的含义后，首先能想到的做法，就是将两个有序数组合并成一个有序数组，然后再根据奇偶来取中位数，思路简单清晰。
    但还有更好的办法，就是并不真正合并两个数组，省去了一大部分的内存空间，我们只需要分别定义两个数组的下标计数器，和一个总长度的计数器，即能实现在不合并数组，只移动下标的处理来得到结果。
     */
